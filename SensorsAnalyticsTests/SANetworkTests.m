...
- (void)testFunctionalManagermentConfig {
    NSString *mainConfigVersion = @"1.2.qqq0";
    NSString *eventConfigVersion = @"1.3.qqq0";
    
    XCTestExpectation *expect = [self expectationWithDescription:@"请求超时timeout!"];
    NSURLSessionTask *task = [self.network functionalManagermentConfigWithRemoteConfigURL:nil mainConfigVersion:mainConfigVersion eventConfigVersion:eventConfigVersion completion:^(BOOL success, NSDictionary<NSString *,id> * _Nonnull config) {
        XCTAssertTrue(success);
        [expect fulfill];
    }];
    NSURL *url = task.currentRequest.URL;
    NSString *string = [NSString stringWithFormat:@"v=%@", mainConfigVersion];
