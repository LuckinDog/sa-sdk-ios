...
- (void)testFixGetQueryItemsWithURLIsEncoded {
    ...
    NSDictionary *items = [NSURL queryItemsWithURLString:urlString];
...
- (void)testGetHostWithURL {
    NSString *host = [NSURL hostWithURL:_url];
    XCTAssertEqualObjects(host, @"sdk-test.datasink.sensorsdata.cn");
}

- (void)testGetHostWithNilURL {
    NSString *host = [NSURL hostWithURL:nil];
    XCTAssertNil(host);
}

- (void)testGetHostWithURLString {
    NSString *host = [NSURL hostWithURLString:@"https://www.google.com"];
    XCTAssertEqualObjects(host, @"www.google.com");
}

- (void)testGetHostWithMalformedURLString {
    NSString *host = [NSURL hostWithURLString:@"google.com"];
    XCTAssertNil(host);
}

- (void)testGetQueryItemsWithURL {
    NSDictionary *items = [NSURL queryItemsWithURL:_url];
    BOOL isEqual = [items isEqualToDictionary:@{@"project": @"zhangminchao", @"token": @"95c73ae661f85aa0"}];
    XCTAssertTrue(isEqual);
}

- (void)testGetQueryItemsWithNilURL {
    NSDictionary *items = [NSURL queryItemsWithURL:nil];
    XCTAssertNil(items);
}

- (void)testGetQueryItemsWithURLString {
    NSDictionary *items = [NSURL queryItemsWithURLString:@"https://sdk-test.datasink.sensorsdata.cn/sa?project=zhangminchao&token=95c73ae661f85aa0"];
    BOOL isEqual = [items isEqualToDictionary:@{@"project": @"zhangminchao", @"token": @"95c73ae661f85aa0"}];
    XCTAssertTrue(isEqual);
}

- (void)testGetQueryItemsWithNilURLString {
    NSDictionary *items = [NSURL queryItemsWithURLString:nil];
    XCTAssertNil(items);
}
