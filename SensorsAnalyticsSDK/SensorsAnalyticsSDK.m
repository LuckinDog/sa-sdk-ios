    ...
    dispatch_async(self.serialQueue, ^{
        if (![loginId isEqualToString:self.loginId]) {
            ...
            if (![loginId isEqualToString:self.anonymousId]) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:SF_TRACK_LOGIN_NOTIFICATION object:nil];
            }
        }
    });
}
...
- (void)logout {
    dispatch_async(self.serialQueue, ^{
        self.loginId = nil;
        [self archiveLoginId];
        [[NSNotificationCenter defaultCenter] postNotificationName:SF_TRACK_LOGOUT_NOTIFICATION object:nil];
    });
}
...
- (void)track:(NSString *)event withProperties:(NSDictionary *)propertieDict withType:(NSString *)type {
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        SALog(@"\n【track event】:\n%@", eventDic);
        [[NSNotificationCenter defaultCenter] postNotificationName:SF_TRACK_EVENT_NOTIFICATION object:nil userInfo:eventDic];
