...
@property (nonatomic, strong) id<SAViewElementTypeProtocol> processor;

@end

@implementation SAViewElementTypeContext
...
- (instancetype)initWithView:(UIView *)view {
    if (self = [super init]) {
        ...
        if ([viewType isEqualToString:@"_UIInterfaceActionCustomViewRepresentationView"] ||
            ...
            self.processor = [[SAAlertElementType alloc] init];
        } else if ([viewType isEqualToString:@"_UIContextMenuActionsListCell"]) {
            self.processor = [[SAMenuElementType alloc] init];
        } else {
            self.processor = [[SAViewElementType alloc] init];
        }
    }
    return self;
}
...
- (NSString *)elementType {
    return [self.processor elementTypeWithView:self.view];
}
