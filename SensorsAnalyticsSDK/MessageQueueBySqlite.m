...
void * SASQLiteQueueTag = &SASQLiteQueueTag;
...
- (id)initWithFilePath:(NSString *)filePath {
    ...
    _sqliteQueue = dispatch_queue_create("com.sensorsdata.analytics.sqliteQueue", DISPATCH_QUEUE_SERIAL);
    dispatch_queue_set_specific(_sqliteQueue, SASQLiteQueueTag, &SASQLiteQueueTag, NULL);
...
- (void)addObejct:(id)obj withType:(NSString *)type {
    dispatch_block_t block = ^(){
        ...
        if (self->_messageCount >= maxCacheSize) {
            ...
            if (ret) {
                self->_messageCount = [self sqliteCount];
            } else {
                SAError(@"touch MAX_MESSAGE_SIZE:%d, try to delete some old events FAILED", maxCacheSize);
                return;
            }
        }
        NSData* jsonData = [self->_jsonUtil JSONSerializeObject:obj];
        NSString* query = @"INSERT INTO dataCache(type, content) values(?, ?)";
        sqlite3_stmt *insertStatement;
        int rc;
        rc = sqlite3_prepare_v2(self->_database, [query UTF8String],-1, &insertStatement, nil);
        if (rc == SQLITE_OK) {
    ...
    if (dispatch_get_specific(SASQLiteQueueTag)) {
        block();
    }else{
        dispatch_sync(self->_sqliteQueue, block);
    }
}
...
- (NSArray *) getFirstRecords:(NSUInteger)recordSize withType:(NSString *)type {
    ...
    dispatch_block_t block = ^{
        ...
        int rc = sqlite3_prepare_v2(self->_database, [query UTF8String], -1, &stmt, NULL);
        if(rc == SQLITE_OK) {
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                @try {
                    ...
                    [contentArray addObject:[[NSString alloc] initWithData:[self->_jsonUtil JSONSerializeObject:eventDict] encoding:NSUTF8StringEncoding]];
                    eventsArr = contentArray;
                } @catch (NSException *exception) {
                    SAError(@"Found NON UTF8 String, ignore");
                    eventsArr = @[];
                }
            ...
            SAError(@"Failed to prepare statement with rc:%d, error:%s", rc, sqlite3_errmsg(self->_database));
            eventsArr = @[];
        }
    };
    if (dispatch_get_specific(SASQLiteQueueTag)) {
...
- (BOOL) removeFirstRecords:(NSUInteger)recordSize withType:(NSString *)type {
    ...
    dispatch_block_t block = ^(){
        NSUInteger removeSize = MIN(recordSize, self->_messageCount);
        ...
        char* errMsg;
        @try {
            if (sqlite3_exec(self->_database, [query UTF8String], NULL, NULL, &errMsg) != SQLITE_OK) {
...
- (NSInteger) sqliteCount {
    ...
    dispatch_block_t block = ^(){
        ...
        int rc = sqlite3_prepare_v2(self->_database, [query UTF8String], -1, &statement, NULL);
        if(rc == SQLITE_OK) {
