    ...
    @try {
        for (SensorsAnalyticsSDK *instance in self.sensorsAnalyticsSDKInstances) {
            if (instance.configOptions.enableTrackAppCrash) {
                ...
                [instance track:SA_EVENT_NAME_APP_CRASHED properties:properties type:SAEventTypeTrack libMethod:SALibMethodAuto];
            }
            if (![instance isAutoTrackEventTypeIgnored:SensorsAnalyticsEventTypeAppEnd]) {
                [SACommonUtility performBlockOnMainThread:^{
                    if (UIApplication.sharedApplication.applicationState == UIApplicationStateActive) {
                        [instance track:SA_EVENT_NAME_APP_END properties:nil type:SAEventTypeTrack libMethod:SALibMethodAuto];
                    }
                }];
            }
